...
mlflow.set_tracking_uri("http://127.0.0.1:5000")  # Change to your MLflow tracking server
...
try:
    ...
    print(f"⚠️ Failed to load model via models:/ URI: {e}")
    try:
...
def load_location_and_property_types():
        ...
        try:
            ...
            cursor.execute(
                "SELECT DISTINCT prop_type, location FROM property_data"
            )
...
try:
    ...
    if not versions:
        print("⚠️ No Production-stage model found; falling back to latest registered version.")
        versions = client.get_latest_versions(model_name)
        if not versions:
...
async def predict_price(input_data: PredictionInput):
    if model is None:
        raise HTTPException(status_code=500, detail="Model not loaded. Check MLflow server and model registry.")
